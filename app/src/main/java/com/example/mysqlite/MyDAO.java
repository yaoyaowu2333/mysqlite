...
public class MyDAO {
    ...
    public Cursor allGroup (){    //查询所有币种
        myDb = dbHelper.getReadableDatabase();
        return myDb.rawQuery("select name from friends group by name",null);
    }
    ...
    public void insertInfo(String name,double price,double money, String flag ){  //插入记录
    ...
    public void updateInfo(String name,double price,double money, String flag,String selId){  //修改记录
        //方法中的第三参数用于修改选定的记录
        ContentValues values = new ContentValues();
        values.put("name", name);
        values.put("price", price);
